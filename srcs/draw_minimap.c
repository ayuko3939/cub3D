...
/*   Updated: 2024/11/26 14:28:18 by yohasega         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void draw_square(t_data *data, int x, int y, int color)
{
	...
	while (i < data->minimap_scale)
	{
		...
		{
			// 四角形の外周は黒色で描画
			if (i == 0 || j == 0 || i == data->minimap_scale - 1 || j == data->minimap_scale - 1)
...
static void	draw_grid(t_data *data)
{
	...
	while (y < data->rows)
	{
		...
		while (x < data->columns - 1)
		{
			...
			// セルが壁なら黒色で四角く塗りつぶす
			if (cell == '1' || cell == ' ')
				draw_square(data, x * data->minimap_scale, y * data->minimap_scale, BLACK);
			// セルが壁なら灰色で四角く塗りつぶす
			else if (cell == '0' || ft_strchr("NSWE", cell))
				draw_square(data, x * data->minimap_scale, y * data->minimap_scale, LIGHT_GREY);
			x++;
		}
		y++;
	}
...
static void	draw_player(t_image *image, t_player *p, int minimap_scale)
{
	...
	while (draw.y < PLYLEN)
	{
		...
		while (draw.x < PLYLEN)
		{
			image_pixel_put(image, player.x + draw.x, player.y + draw.y, ORANGE);
			draw.x++;
		}
		draw.y++;
	}
